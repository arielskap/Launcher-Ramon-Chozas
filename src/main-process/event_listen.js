...
ipcMain.on('open-app', (event, app_name) => {
    ...
    if (!json_app) {
        return event.reply('reply-open-app', {code : 400, message : "app_not_exist", app_name : app_name }); 
    }

    if (!one_app){
        return event.reply('reply-open-app', {code : 400, message : "app_in_use", app_name : app_name }); 
    }
    ...
    return event.reply('reply-open-app', {code : 200,message : "app_open", app_name : app_name }); 
});
